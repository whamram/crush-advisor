...
ADVISOR_OUTPUT=$("$SCRIPT_DIR/session-dump.sh" "${CRUSH_SESSION_ID:-}" "${CRUSH_CWD:-$PWD}" \
  ...
  "You are advising another agent. Here is its session transcript. Provide a concise response, providing a concrete plan forward, risks and edge cases the model may run into, and any necessary corrections to the agents current thought process or actions.")
